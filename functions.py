...
def sendAndCritical(msg):
    logger.critical(msg)
    if CALL_ALARM: callAlarm(strategyName=STRATEGY_NAME, content=msg)
    sendMixin(msg)
...
def getOpenSignal(exchangeId, klines, selectNum, selectFactor, selectPeriod, filterFactor, filterLevel, filterPeriod):
    ...
    if longCoins:
        ...
        for symbol,df in klines.items():
            ...
            if isOk: filterCoins.append(symbol)
...
def placeBatchOrderClose(exchange, symbols, markets):
    ...
    for i in range(0, len(symbols), 5):
        ...
        ordersTotal += orders
        try:
            ordersStr = [exchange.encode_uri_component(exchange.json(order), safe=",") for order in orders]
    ...
    for index,r in enumerate(responsesTotal):
        for i in range(MAX_TRY):
            ...
            if "orderId" in r:
                orderInfo = exchange.fapiPrivateGetOrder({"orderId":r["orderId"], "symbol":r["symbol"]})
                ...
                sendAndCritical(f"{STRATEGY_NAME} placeBatchOrderClose({orders[index]['symbol']})在批量平仓中失败，请检查。{r}")
    ...
    for index,r in enumerate(responsesTotal):
        for i in range(MAX_TRY):
            ...
            if "orderId" in r:
                ...
                sendAndCritical(f"{STRATEGY_NAME} placeBatchOrderClose({orders[index]['symbol']})在批量买入中失败，请检查。{r}")
...
def openPosition(exchangeId, markets, openPositions, openFactor, openLevel, openPeriod, filterFactor, filterLevel, filterPeriod):
    ...
    selectNum = SELECTION_NUM - len(openedSymbols)
    longCoins = longCoins[:selectNum]
    opened = []
